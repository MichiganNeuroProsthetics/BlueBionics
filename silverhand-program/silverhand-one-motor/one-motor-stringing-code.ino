...
#define THERMISTOR A7 //thermistor - check heat of pcb?
...
#define DEFAULT_THRESH 30 //(0,1024)
...
void loop() {
  ...
  int thermistorValue = analogRead(THERMISTOR);
  float resistance = R0 * (1023/(float)thermistorValue - 1.0);
  float inverseT = (1/T0) + (log(resistance/R0)/B);
  float T = 1/inverseT;
  T -= 273.15; //what is this value?
  if(T > 100){
      while (1) {
        delay(DELAY_BLINK);
        // Flash red
        ...
        digitalWrite(LED_PIN_G, LOW);
        digitalWrite(LED_PIN_B, LOW);
      } //flash red 5 times, then shut down
