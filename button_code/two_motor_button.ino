...
#define MODE_BUT_PIN A5 // Potentiometer for mode control
...
// button control of mode selection
void updateMode () {
  buttonState = digitalRead(MODE_BUT_PIN);
  ...
  if (buttonState != lastState) {
    if (buttonState == HIGH){
      ...
      if (buttonMode == 5 || buttonMode == 0) {
        //MODE 1 (ti): blinks white twice
        writeColors(LOW, LOW, LOW);
        delay(200);
        ...
        //MODE 4 (grab): blinks brown? twice
        writeColors(LOW, LOW, LOW);
        delay(200);
        ...
        //MODE 5 (any): blinks yellow twice
        writeColors(LOW, LOW, LOW);
        delay(200);
